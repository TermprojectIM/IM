...
public class frmHelp extends JFrame {
	...
	public frmHelp(Main main, String day, String begin, String finish) {
		...
		btnMake = new JButton(new ImageIcon("img/btnMake.png"));
		btnDel = new JButton(new ImageIcon("img/btnDel.png"));
		btnMake.setBounds(1024, 700, 230, 65);
		btnDel.setBounds(1255, 700, 230, 65);
		btnHome.setBounds(960, 12, 45, 45);
		btnBlind(btnMake);
		btnBlind(btnDel);
		btnBlind(btnPut);
		btnBlind(btnHome);
		...
		btnMake.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < cnt; i++) {
					String temp[][] = main.readHelpInfo(result[i][1]);//그 지역에 관한 헬프를 읽어서 템프에 담는다.
					hi[i] = new HelpInfo(result[i][0], result[i][1], Integer.parseInt(temp[0][0]), temp[0][1],
							temp[0][2], temp[0][3], temp[0][4]); //각각의 레코드를 hi[i]안에 담는다
				}
				frmMakeChartHelp fmch = new frmMakeChartHelp(hi, day, begin, finish, main);
			}
		});

		add(setJLayered(backGround, imgM, imgM.btnSearch, search, box, btnPut, info.scrollpane, scrollpane, btnMake,
				btnDel, btnHome));
		setVisible(true);
	}
